...
the appointments app, not a shared "common" module) so this module owns its own
...
_STYLES = {
    "section_title": ParagraphStyle(
        "section_title", fontName="Helvetica-Bold", fontSize=12.5,
        textColor=NAVY, leading=15,
    ),
    "meta": ParagraphStyle(
        "meta", fontName="Helvetica", fontSize=9, textColor=GRAY_TEXT, leading=13,
    ),
    "page_title": ParagraphStyle(
        "page_title", fontName="Helvetica-Bold", fontSize=19, textColor=NAVY, leading=22,
    ),
    "card_label": ParagraphStyle(
        "card_label", fontName="Helvetica-Bold", fontSize=7, textColor=GRAY_TEXT,
        leading=9,
    ),
    "card_sub": ParagraphStyle(
        "card_sub", fontName="Helvetica", fontSize=7, textColor=GRAY_TEXT, leading=9,
    ),
    "th": ParagraphStyle(
        "th", fontName="Helvetica-Bold", fontSize=8.5, textColor=WHITE, leading=10,
    ),
    "td": ParagraphStyle(
        "td", fontName="Helvetica", fontSize=8.3, textColor=colors.HexColor("#1f2937"),
        leading=10,
    ),
    "pill": ParagraphStyle(
        "pill", fontName="Helvetica-Bold", fontSize=7.5, alignment=TA_CENTER, leading=9,
    ),
    "empty": ParagraphStyle(
        "empty", fontName="Helvetica-Oblique", fontSize=10, textColor=GRAY_TEXT,
        alignment=TA_CENTER,
    ),
}
...
def card_value_style(color_hex):
    return ParagraphStyle(
        f"card_value_{color_hex}", fontName="Helvetica-Bold", fontSize=17,
        textColor=colors.HexColor(color_hex), leading=20,
    )

def section_heading(number, title, width):
    badge = Drawing(22, 20)
    badge.add(Circle(10, 9, 9.2, fillColor=NAVY, strokeColor=None))
    badge.add(String(10, 5.3, str(number), fontSize=10.5, fillColor=WHITE,
...
def kpi_card_grid(cards, total_width, columns=4):
    ...
    grid = Table(grid_rows, colWidths=[col_width] * columns, spaceAfter=4, spaceBefore=SECTION_GAP)
    style = [
        ("VALIGN", (0, 0), (-1, -1), "TOP"),
...
def _status_pill(value):
    ...
    style = ParagraphStyle("pill_x", parent=_STYLES["pill"], textColor=colors.HexColor(fg))
...
def data_table(headers, rows, col_widths=None, status_cols=None, total_width=None):
    ...
    if not rows:
        ...
        t = Table([empty_row], colWidths=[total_width] if total_width else None, spaceBefore=SECTION_GAP)
        t.setStyle(TableStyle([
...
def _draw_header(canvas, doc, report_title, report_subtitle, organization, generated_at):
    ...
    if RLImage and os.path.exists(ISF_LOGO):
        try:
            pad = 2
            canvas.drawImage(
                ISF_LOGO, logo_x + pad, logo_y + pad,
                width=logo_size - 2 * pad, height=logo_size - 2 * pad,
...
def _draw_footer_full(canvas, organization, generated_at):
    ...
    if RLImage and os.path.exists(ORCHASP_LOGO):
        try:
            canvas.drawImage(
                ORCHASP_LOGO, lx, ly, width=logo_size, height=logo_size,
                preserveAspectRatio=True, mask="auto",
            )
        except Exception:
            pass
    ...
    canvas.drawRightString(W - SIDE_MARGIN, line_y - 8, f"{org_name}  |  {generated_at}")
...
def generate_module_report_pdf(
    ...
    def on_first_page(canvas, doc):
        _draw_header(canvas, doc, report_title, report_subtitle, organization, generated_at)
    ...
    first_frame = Frame(
        SIDE_MARGIN, FOOTER_HEIGHT + 4,
        content_width, PAGE_H - HEADER_HEIGHT - FOOTER_HEIGHT - 12,
        id="content_first",
    )
    later_frame = Frame(
        SIDE_MARGIN, FOOTER_HEIGHT + 4,
        content_width, PAGE_H - CONT_TOP_MARGIN - FOOTER_HEIGHT - 4,
        id="content_later",
    )
    ...
    org = organization or {}
    story.append(Paragraph(org.get("name") or "All Organizations", _STYLES["page_title"]))
