"""
Shared IndusServiceFlow report styling for ReportLab-generated PDFs.

Every module export (employees, customers, queue, services, appointments)
uses this module so all PDFs share one visual language: the navy header
banner + teal accent bar, the numbered section headings, the pastel KPI
cards, and the Orchasp footer -- the same look as the platform's
Organization Report.
...
def kpi_card_grid(cards, total_width, columns=4):
    ...
    grid = Table(grid_rows, colWidths=[col_width] * columns, spaceAfter=4)
    style = [
        ("VALIGN", (0, 0), (-1, -1), "TOP"),
...
def data_table(headers, rows, col_widths=None, status_cols=None, total_width=None):
    ...
    if not rows:
        ...
        t = Table([empty_row], colWidths=[total_width] if total_width else None)
        t.setStyle(
            TableStyle(
                [
...
def _draw_footer(canvas, doc, organization, generated_at, printed_on):
    canvas.saveState()
    W, H = PAGE_SIZE
    ...
    canvas.drawCentredString(W / 2, 8, f"Page {canvas.getPageNumber()}")
...
def generate_module_report_pdf(
    ...
    def on_page(canvas, doc):
        ...
        _draw_footer(canvas, doc, organization, generated_at, printed_on)
