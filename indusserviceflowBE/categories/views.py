...
def add_category(request):
    ...
    if serializer.is_valid():
        ...
        log_action(
            ...
            "Create",
            "Categories",
            target_info=serializer.data.get("category_name"),
        )

        return Response(
            {"message": "Category Added Successfully", "data": serializer.data},
...
def get_categories(request):
    ...
    categories = Category.objects.annotate(
        organizations_count=Count(
            "organizations", filter=Q(organizations__is_deleted=False)
        )
    )
    ...
    if search:
        categories = categories.filter(Q(category_name__icontains=search))
...
def get_category_by_id(request, id):
    ...
    return Response(
        {"message": "Category Retrieved Successfully", "data": serializer.data},
        status=status.HTTP_200_OK,
    )
...
def delete_category(request, id):
    ...
    category.updated_by = request.user.username
    category.save()

    log_action(request.user, "Delete", "Categories", target_info=category.category_name)
...
# ==========================================================
# Export Categories (CSV / Excel / PDF)
# ==========================================================
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def export_categories(request):
    ...
    report_title = "CATEGORIES REPORT"
    ...
    if export_format == "csv":
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="categories_{date_str}.csv"'
        )
        ...
        for sno, category in enumerate(categories, 1):
            writer.writerow(
                [
                    sno,
                    ...
                    category.created_by,
                    (
                        category.created_on.strftime("%Y-%m-%d %H:%M:%S")
        ...
        log_action(request.user, "Export", "Categories")
        return response
        ...
        for sno, category in enumerate(categories, 1):
            ...
            sheet.cell(row=row_num, column=2).value = category.category_name
            sheet.cell(row=row_num, column=3).value = category.organizations_count
            sheet.cell(row=row_num, column=4).value = category.status
            sheet.cell(row=row_num, column=5).value = category.created_by
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="categories_{date_str}.xlsx"'
        )
        ...
        active_count = sum(
            1 for c in categories_list if (c.status or "").strip().lower() == "active"
        )
        inactive_count = total_categories - active_count
        total_orgs_linked = sum(c.organizations_count for c in categories_list)
        ...
        rows = [
            ...
            for sno, category in enumerate(categories_list, 1)
        ]
        ...
        response = _build_list_report_pdf(
            ...
            table_section_title="Category Directory",
            headers=headers,
            rows=rows,
            status_col_index=3,
            total_count=total_categories,
            generated_by_username=request.user.username,
            filename=f"categories_{date_str}.pdf",
        )
